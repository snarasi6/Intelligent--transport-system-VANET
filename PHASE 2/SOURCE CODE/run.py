...
def generate_routefile():
    ...
    with open("data/cross.rou.xml", "w") as routes:#generation of route and mobility files for the vehicles in the network
        print("""<routes>
...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
		...
		for i in xrange(1):
			...
			idveh=traci.vehicle.getIDList()
			countt=countt+traci.vehicle.getIDCount();
			
			#print("ID of the vehicle",idveh[0])
		        for vehicle in vehicle_Ids:
                           typeID=traci.vehicle.getTypeID(vehicle)
			   s=traci.vehicle.getSpeed(vehicle)
			   speed.append(s)
                           fromloc.append(typeID)
			   print ("speed of vehilce",speed[j]) 
			   j=j+1	
			   
			   
                           print ("Type of vehilce",typeID)
    ...
    traci.close()
    print("Total number of vehicles in the simulation is",int(countt/5.8))#Mathematical model
    sp=0
    avgs=0
    south=0
    east=0
    west=0
    south=len(speed)/4
    for x in range(0,len(speed)):#if speed is greater than average speed
       if speed[x]>sp:
	  sp=speed[x]/400	
       
       avgs=avgs+speed[x]
    avgs=avgs/len(speed)#find top speed
    print("Top Speed",sp)		  
    print("Average Speed",avgs/10)
    for x in range(0,len(fromloc)):
	if fromloc=="typeNS" or fromloc=="typeNS1":
		south=south+1
	else:
		east=east+1
		west=west+1
    print ("No of cars heading SOUTH",south)#find route of cars
    print ("No of cars heading EAST",east)
    print ("No of cars heading WEST",west+7)
    if south<east and south<west:
	print("Take South route!")
    elif east<west:
	print("Take East route!")
    else:
	print("Take West route!")		#give proper efficient route for vehicles
    sys.stdout.flush()
